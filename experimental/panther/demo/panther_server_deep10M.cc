...
const std::vector<int64_t> k_c = {206094, 113489, 56939, 31248,
                                  18921,  18920,  43180};
...
const size_t total_points_num = 9990000;
...
const uint32_t total_cluster_size = 445611;
const uint32_t sum_k_c = 488791;

auto cluster_data = read_data(sum_k_c, dims, "dataset/deep10M_centroids.txt");
auto stash = read_data(k_c[k_c.size() - 1], 1, "dataset/deep10M_stash.txt");
auto ps = read_data(total_points_num, dims, "dataset/deep10M_dataset.txt");
auto ptoc = read_data(total_cluster_size, max_cluster_points,
