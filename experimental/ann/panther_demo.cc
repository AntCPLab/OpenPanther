    ...
    for (uint64_t j = 0; j < element_size; j++) {
      if (num_points == dims) {
        ...
        if (count == 6) {
          count = 0;
          id_point++;
          if (id_point == max_cluster_points) {
            id_point = 0;
          }
          num_points = 0;
        }
...
spu::seal_pir::MultiQueryServer PrepareMpirServer(
    ...
    std::shared_ptr<yacl::link::Context>& lctx) {
  auto db_bytes = PirData(ele_number, ele_size);
  double factor = 1.5;
  size_t hash_num = 3;
  spu::psi::CuckooIndex::Options cuckoo_params{batch_number, 0, hash_num,
                                               factor};
...
int main(int argc, char** argv) {
  ...
  // p, ID(p), p^2
  // ID(p) and p^2 saved in 3 * 8 bits, which can support 2^24 points
  size_t ele_size = (dims + 2 * 3) * max_cluster_points;

  SPDLOG_INFO(
      "Batch query size: {}, Cluster size: {}, Element size (coeff size): {}",
      batch_size, cluster_num, ele_size);

  // caculate the bit width of cluster id and points id
  ...
  // Limited by the implementation of SPU
  // Prepare produce 10485760 OT for each thread, which can't be consumed.
  // This time can be amortized in many queries.
  ...
  if (rank == 0) {
    ...
    // TODO: how to pad?
    auto dis = dis_client.RecvReply({total_bin_number, max_bin_size},
                                    cluster_data.size());
    ...
    auto _out = batch_argmax.ComputeWithIndex(dis, index, logt, cluster_dc_bits,
                                              total_bin_number, max_bin_size);
    ...
    auto fix_pir =
        FixPirResult(pir_res, pir_logt, pir_fixt, logt,
                     pir_res.size() * max_cluster_points, dims + 6, kctx);
    ...
    SPDLOG_INFO("Centeres {} {}:", cluster_num, dims);
    // auto ps = cluster_data;
    ...
    auto dis = dis_server.DoDistanceCmp(cluster_data, q,
                                        {total_bin_number, max_bin_size});
