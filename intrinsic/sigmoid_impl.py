...
def sigmoid(input):
    ...
    return _sigmoid_prim.bind(input)
...
def _sigmoid_lowering(ctx, input):
...
def _sigmoid_jvp(args, tangents):
...
# *********************************************
_sigmoid_prim = core.Primitive("sigmoid")
# Change this to True if there are more than 1 output
_sigmoid_prim.multiple_results = False
_sigmoid_prim.def_impl(partial(xla.apply_primitive, _sigmoid_prim))
_sigmoid_prim.def_abstract_eval(_sigmoid_abstract)

mlir.register_lowering(_sigmoid_prim, _sigmoid_lowering)

# Connect the JVP and batching rules
ad.primitive_jvps[_sigmoid_prim] = _sigmoid_jvp
batching.primitive_batchers[_sigmoid_prim] = _sigmoid_batch
